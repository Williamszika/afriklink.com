...
/**
 * Comparateur de produits — côté visiteur, par cookie first-party (identifiants
 * publics). Limité à MAX produits (le plus ancien saute au-delà).
 */
final class Compare
{
    private const COOKIE = 'afk_compare';
    private const MAX    = 4;
    ...
    /** Bascule un produit ; renvoie true s'il est désormais dans le comparateur. */
    public static function toggle(string $pid): bool
    {
        ...
        if ($idx !== false) {
            ...
            $ids[] = $pid;        // ajout en fin ; le slice ci-dessous fait sauter le plus ancien
            $now = true;
        }
